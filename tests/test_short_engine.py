...
def test_enter_short_on_failed_pullback():
    ...
    ep = ema(cl, P.ema_pullback)
    candles = mk(closes)
    i = len(closes) - 1
    # dernière bougie = rejet : mèche au-dessus de l'EMA pullback, clôture dessous, bougie baissière
    candles[-1] = replace(
        candles[-1], high=Decimal(str(ep[i] + 5)), open=candles[-1].close + Decimal("3")
    )
    d = evaluate_short(candles, None, P)
...
def test_exit_short_stop():
    pos = PositionState(
        Decimal("100"), Decimal("95"), Decimal("10"), 1
    )  # stop sous le close -> hit
...
def test_exit_short_trend_recovered():
